...
int len=0;
int hdr_len=0;
linkaddr_t  * dest_address; //Destination Address.
linkaddr_t *update_neighbor_node;
...
static void on_for_packet(void *ptr){
	on();
	waiting_for_packet=1;
  clock_time_t next_time = (CLOCK_SECOND * ((RTIMER_NOW() + 4*DEFAULT_ON_TIME) - RTIMER_GUARD_TIME)) / RTIMER_ARCH_SECOND;
...
/*-------------------------------------------SLEEP PERIOD-------------------------------------------------------------------------*/
        clock_time_t timer_on_pack= (CLOCK_SECOND * rx_ies.ie_rz_time/240)/RTIMER_ARCH_SECOND;
