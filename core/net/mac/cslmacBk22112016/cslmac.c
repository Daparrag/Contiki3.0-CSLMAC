...
//   // Used in case of unicast/syncronous-TX
...
rtimer_clock_t remaing_time = RTIMER_NOW();
uint8_t more_frame;
uint8_t async_dataframe_tx_wait=0;
//uint8_t collisions=0;
uint8_t r_seqno=0;
uint8_t got_ack=0;
uint8_t rx_frame_type=0;
...
/*-----------------------------------Transmission_phase--------------------------------------------------*/	
			if(requiere_wakeup){
				...
				*/
				...
				if(is_synchronous){
						...
						powercycle_turn_radio_off();
						we_are_sending=1;
						while(RTIMER_CLOCK_LT(RTIMER_NOW(),t + 9 * neighbor_time/10));
						//_nextSamplePeriod= RTIMER_NOW() + 6 * neighbor_time / 10 + 32 * DEFAULT_ON_TIME / 10;
						_nextSamplePeriod= RTIMER_NOW()+ 2 * neighbor_time/10 /*+ 5 * DEFAULT_ON_TIME / 2*/;
						//PRINTF("wakeup for neighbor_time (%i)\n",_nextSamplePeriod - RTIMER_NOW());
						we_are_sending=0;
						PRINTF("cslmac: synchronous parameters neighbor_time(%u), _nextSamplePeriod(%u)\n",neighbor_time,_nextSamplePeriod);
					
				}else{
						_nextSamplePeriod= RTIMER_NOW() + cslmac_config.maccslperiod- 4 * DEFAULT_ON_TIME;
				}
			...
			if(requiere_ack){
						...
						if(len!=0){
							if(Got_frame_type(p_ack_buf)==0x2){
								if(tsch_packet_parse_eack(ack_buf,sizeof(ack_buf),r_seqno,&ack_frame,&r_ies,(uint8_t *)hdr_len)>0){
									...
									clock_time_t next_wakeup=RTIMER_NOW()-remaing_time;
									if(next_wakeup<cslmac_config.maccslperiod){
										next_wakeup = cslmac_config.maccslperiod - next_wakeup;
									}else{
										next_wakeup  = next_wakeup - cslmac_config.maccslperiod;
									}
									PRINTF("the remaining time for sender (%u) in on_phase (%u)\n",next_wakeup,on_phase);
									if(on_phase) next_wakeup = next_wakeup + DEFAULT_OFF_TIME;
									if(r_ies.CSL_period<next_wakeup){
											r_ies.CSL_period = cslmac_config.maccslperiod -((next_wakeup)-r_ies.CSL_period);
										}else{
											r_ies.CSL_period = cslmac_config.maccslperiod + (r_ies.CSL_period-(next_wakeup));
										}
									PRINTF("cslmac: neighbor next_wakeup period (%u)  \n", r_ies.CSL_period);
				...
				watchdog_start();
				/*if(!got_ack && update_neighbor_node == NULL){
					dest_address=queuebuf_addr(packet, PACKETBUF_ADDR_RECEIVER);
					if(is_neighbor_in_list(dest_address)){
					remove_neighbor (dest_address);
					}
				}else{
					remove_neighbor (update_neighbor_node);
...
sync_powercycle(void*ptr)
{
	...
	if(someone_is_sending > 0){
		...
		PT_YIELD_UNTIL(&pt2,timer_expired(&cpowercycle_ctimer));
...
static void on_for_packet(void *ptr){
	on();
	waiting_for_packet=1;
	rtimer_set(&ct, (RTIMER_NOW() + cslmac_config.ack_wait_time - 4 * DEFAULT_ON_TIME / 10), 1, off_for_packet, NULL);
...
//clock_time_t remaing_time=RTIMER_NOW();
...
/*-------------------------------------------SLEEP PERIOD-------------------------------------------------------------------------*/
				rtimer_clock_t now;
				now=RTIMER_NOW();
				if(now + rx_ies.ie_rz_time - 3 * DEFAULT_ON_TIME / 2 >= 0){
					rtimer_set(&ct, now + rx_ies.ie_rz_time - 3 * DEFAULT_ON_TIME / 2 , 1, on_for_packet, NULL);
					//wait_framer=1;
				}else{
					rtimer_set(&ct, 1, 1, on_for_packet, NULL);
					//wait_framer=1;
				}
							...
							linkaddr_cmp(packetbuf_addr(PACKETBUF_ADDR_RECEIVER),
								&linkaddr_null)){
										...
										/*1.build the eack*/
											...
											clock_time_t next_wakeup=etimer_expiration_time(&cpowercycle_ctimer.etimer);
