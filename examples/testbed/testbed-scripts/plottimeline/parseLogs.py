...
def doAppDrop(packetId, status):
    ...
    if status == 2: # noack
        if packetId in appDataStats:
            appDataStats[packetId]['refmoduleInfo']['macDrop'] = True
    elif status == 4: # error (e.g. TSCH is not associated, out-of-memory error)
        if packetId in appDataStats:
            appDataStats[packetId]['refmoduleInfo']['macError'] = True
...
#---- App: Sending -------------------------------------------------------------------------------------------------------------
    if log.startswith('sending'):
        ...
        # we have received this packet earlier
...
#---- RPL: state overview  -------------------------------------------------------------------------------------------------------------
    ...
    if res:             
        ...
        rplNeighbors = int(res.group(3))
        ds6neighbors = int(res.group(4))  
        return {'event': 'status', 'rank': rank, 'dioint': dioint, 'neighbors': rplNeighbors }
...
#---- TSCH link: Rx -------------------------------------------------------------------------------------------------------------
        res = re.compile('([ub]c)-([01]) (\d+) rx (\d+)(.*)$').match(log)
...
#---- TSCH link: Tx -------------------------------------------------------------------------------------------------------------                
        res = re.compile('([ub]c)-([01]) (\d+) tx (\d+), st (\d+)-(\d+)(.*)$').match(log)
        if res:
            is_unicast = res.group(1) == "uc"
            is_data = int(res.group(2))
            datalen = int(res.group(3))
...
def doParse(file, sinkId):
    ...
    for line in open(file, 'r').readlines()[-25000:]:
        log = None
        module = None
        ...
        if module == None or log == None:
            ...
            if packetInfo and packetId in appDataStats:
