...
def generateTimelineFileVector(dir, parsed, txOnly=False):
    ...
    for asn in timeline.keys():
        ...
        for node in asnEvents:
            ...
            time = (asn - firstAsn) * 0.015
            if time > 0 and time < 10 and asnEvents[node]['packet'] != None:
                packet = asnEvents[node]['packet']['id']
                if not (packet,event) in per_packet_data_x:
                    per_packet_data_x[(packet,event)] = []
                    per_packet_data_y[(packet,event)] = []
                per_packet_data_x[(packet,event)].append(asn-firstAsn)
                per_packet_data_y[(packet,event)].append(id_to_index[node])
    ...
    for k in data_x.keys():
        ...
        data_x[k] = data_x[k] + xshift
    for k in per_packet_data_x.keys():
        per_packet_data_x[k] = array(per_packet_data_x[k])
        per_packet_data_y[k] = array(per_packet_data_y[k])
        per_packet_data_x[k] = per_packet_data_x[k] * 0.015
        per_packet_data_x[k] = per_packet_data_x[k] + xshift
    
    packetlist = map(lambda x: x[0], filter(lambda x: x[1] == "Rx",per_packet_data_x.keys()))
    print packetlist
    hot_packet = packetlist[4]
    print per_packet_data_x[(hot_packet,"Tx")]
    print per_packet_data_y[(hot_packet,"Tx")]
    ...
    if zoomOut:
      ax.axis(ymin=0,ymax=len(nodeIDs),xmin=0,xmax=60)
      sizea = 3
      sizeb = 1
    else:
      ax.axis(ymin=0,ymax=len(nodeIDs),xmin=0,xmax=4)
      sizea = 5
      sizeb = 2
         
    colors = ['#FF9900', '#00A876', '#0a51a7', '#FF5900', '#8FD9F2', 'black']
    ax.scatter(data_x[(0,"Tx")], data_y[(0,"Tx")], marker='s', edgecolor='none', color=colors[0],s=sizea, label="EB Tx")
    ax.scatter(data_x[(0,"Rx")], data_y[(0,"Rx")], marker='o', edgecolor='none', color=colors[0],s=sizeb, label="EB Rx")
    ax.scatter(data_x[(1,"Tx")], data_y[(1,"Tx")], marker='s', edgecolor='none', color=colors[1],s=sizea, label="Broadcast Tx")
    ax.scatter(data_x[(1,"Rx")], data_y[(1,"Rx")], marker='o', edgecolor='none', color=colors[1],s=sizeb, label="Broadcast Rx")
    ax.scatter(data_x[(2,"Tx")], data_y[(2,"Tx")], marker='s', edgecolor='none', color=colors[2],s=sizea, label="Unicast Tx")
    ax.scatter(data_x[(2,"Rx")], data_y[(2,"Rx")], marker='o', edgecolor='none', color=colors[2],s=sizeb, label="Unicast Rx")
    
    plt.gca().invert_yaxis()
    if not zoomOut:
        ax.set_xlabel("Time (s)", fontsize=16)
    ax.set_ylabel("Node ID", fontsize=16)
    if zoomOut:
        ax.legend(bbox_to_anchor=(0., 1.02, 1., .102), loc=3,
           ncol=6, borderaxespad=0., prop={'size':12}, markerscale=2)
    #plt.axis('off')
    fig.savefig('timeline.pdf' if not zoomOut else 'timeline_zoomed_out.pdf', format='pdf', bbox_inches='tight', pad_inches=0)
